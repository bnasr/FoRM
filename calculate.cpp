...
double DewPointTemperature(double T, double RH){
	...
	double c = 257.14;
	double d = 234.5;
	double Gamma = log(RH*exp((b - T/d)*(T/(c + T))));
...
void CalculateTemporal(InputData Input, double **DOY, double **Tair, double **Tair4, double **Tsnow4, double **DeclinationAngle, double **HourAngle,double **AltitudeAngle, double **AzimuthAngle, double **ExtraterrestrialSolarRadiation, double **tb, double **td, int **month, ofstream *fsLOG){
	...
	*month=new int[NumberOfTimeSteps];
	...
	// ---------------temporal variables
	for(int TimeStep=0;TimeStep<NumberOfTimeSteps;TimeStep++){
		(*DOY)[TimeStep]=mod((365+Input.DOY_START+(double)TimeStep/Input.NUMBER_OF_TIMESTEP_IN_A_DAY),365);
		...
		double Tdp=DewPointTemperature((*Tair)[TimeStep],Input.RH[(int)(*DOY)[TimeStep]]);
		(*Tsnow4)[TimeStep]=pow(min(Tdp,0)+273.15+Input.SNOW_DEL_TEMPERATURE,4);
...
//------------SHORTWAVE
		#pragma omp parallel for private(i,j,P,PL) shared(expPathLengthAVG,sdir,sdif,ssnow) 
		for(int TimeStep=0;TimeStep<NumberOfTimeSteps;TimeStep++){
			...
			for( i=0;i<Input.NUMBER_OF_SPACE_GRID;i++){
				for( j=0;j<Input.NUMBER_OF_SPACE_GRID;j++){
					P.x=dx*(i-iCenter);
					P.y=dy*(j-jCenter);
					PL=(*AltitudeAngle)[TimeStep]<=0?9999999:PathLength( Input.Tree, TreeInterval, P, (*AltitudeAngle)[TimeStep], (*AzimuthAngle)[TimeStep], Slope, Aspect);
					if(Input.PL_OUT==1){
						//#pragma omp critical
		...
		//scout<<"**********"<<(*PLMatrixPos)<<"\n";
		double SDIR=0, SDIF=0, SSNOW=0, SF=1;
		#pragma omp parallel for shared(NumberOfTimeSteps, expPathLengthAVG) reduction(+:SDIR) reduction(+:SDIF) reduction(+:SSNOW) reduction(+:SF)
		for(int TimeStep=0;TimeStep<NumberOfTimeSteps;TimeStep++){
			SDIR+=sdir[TimeStep]/NumberOfTimeSteps;
			SDIF+=sdif[TimeStep]/NumberOfTimeSteps;
			SSNOW+=ssnow[TimeStep]/NumberOfTimeSteps;
			SF-=expPathLengthAVG[TimeStep]/NumberOfTimeSteps;
